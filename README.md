...
Channels are registered at startup and scope the marketer-facing Pinned editor
...
Register channels with the fluent `AddSearchChannel(...)` extension on the
...
services.AddGraphSearchtools(options =>
    ...
    .AddSearchChannel("alloy-products", p => p
...
When no channels are registered the addon synthesises a single **Generic**
channel so zero-config installs keep working: the Channels index shows one
...
channels and acts as the catchment for orphan collections that don't match a
declared `UsesPinnedKey` formula.

Pinned results are scoped to channels. Marketers reach the editor at
`/EPiServer/cms/graphsearchtools/channels/{key}`; the legacy
`/EPiServer/cms/graphsearchtools/Pinned` URL 301-redirects to the Channels
...
synonyms are tenant-global and aren't scoped to channels.
