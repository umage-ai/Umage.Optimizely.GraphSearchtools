...
/// </summary>
/// <remarks>
/// This is the v0.5 design's aggregate-first ingest, distinct from the legacy
/// <see cref="TelemetryApiController"/> (Phase 4 foundation, server-side
/// auth-gated, raw-row writes). Both coexist during the dual-running migration
/// window per <c>docs/search-telemetry-design.md</c> §8.
/// </remarks>
[ApiController]
[Route("api/telemetry")]
public sealed class TelemetryBeaconApiController : ControllerBase
{
    ...
    private readonly ILogger<TelemetryBeaconApiController> _logger;

    public TelemetryBeaconApiController(
        IServiceProvider services,
        TelemetryAbuseGuard guard,
        IOptions<GraphSearchtoolsOptions> options,
        ILogger<TelemetryBeaconApiController> logger)
    {
        _services = services;
        _guard = guard;
