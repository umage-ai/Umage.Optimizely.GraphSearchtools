...
public class SearchLogsApiController : Controller
{
    ...
    /// </summary>
    [HttpGet]
    public async Task<IActionResult> Top([FromQuery] DateTime? since, [FromQuery] int? take, [FromQuery] string? profileKey = null, CancellationToken cancellationToken = default)
    {
        if (!HasAccess()) return Forbid();
        try
        {
            return Ok(await _service.TopPhrasesAsync(since, take, profileKey, cancellationToken));
        }
        catch (Exception ex)
        {
    ...
    /// </summary>
    [HttpGet]
    public async Task<IActionResult> ZeroResults([FromQuery] DateTime? since, [FromQuery] int? take, [FromQuery] string? profileKey = null, CancellationToken cancellationToken = default)
    {
        if (!HasAccess()) return Forbid();
        try
        {
            return Ok(await _service.ZeroResultPhrasesAsync(since, take, profileKey, cancellationToken));
        }
        catch (Exception ex)
        {
            return HandleError(ex);
    ...
    /// </summary>
    [HttpGet]
    public async Task<IActionResult> LowCtr([FromQuery] DateTime? since, [FromQuery] int? take, [FromQuery] string? profileKey = null, CancellationToken cancellationToken = default)
    {
        if (!HasAccess()) return Forbid();
        try
        {
            return Ok(await _service.LowCtrPhrasesAsync(since, take, profileKey, cancellationToken));
        }
        catch (Exception ex)
        {
            return HandleError(ex);
